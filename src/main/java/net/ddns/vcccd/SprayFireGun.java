...
public class SprayFireGun implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            ItemStack LazerGun = new ItemStack(Material.GOLDEN_HORSE_ARMOR);

            //Adopts the item meta and changes the display name
            ItemMeta LazerGunData = LazerGun.getItemMeta();
            LazerGunData.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&f&lSuper LaZer&6&lGun"));
